  ...
  if (lines.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-ink-300 bg-ink-50 px-6 py-20 text-center">
        ...
        <p className="mx-auto mt-2 max-w-sm text-sm text-ink-600">Browse graded key issues in the store, or book a grading, pressing or appraisal service.</p>
              ...
              <div className="min-w-0 flex-1">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-[12px] uppercase tracking-wide text-ink-500">{line.kind === "comic" ? "Collectible comic" : "Service booking"}</p>
                  </div>
                  <p className="font-display text-lg font-semibold tabular-nums text-ink-950">{formatExact(line.price * line.qty)}</p>
                </div>
                <div className="mt-4 flex flex-wrap items-center gap-4">
                  <div className="inline-flex items-center rounded-lg border border-ink-300">
                    <button type="button" onClick={() => setQty(line.id, line.qty - 1)} className="h-9 w-9 text-ink-600 hover:bg-ink-100" aria-label={`Decrease quantity of ${line.name}`}>
                      −
                    </button>
                    <span className="w-9 text-center text-sm font-semibold tabular-nums" aria-live="polite">
                      {line.qty}
                    </span>
                    <button type="button" onClick={() => setQty(line.id, line.qty + 1)} disabled={line.qty >= line.maxQty} className="h-9 w-9 text-ink-600 hover:bg-ink-100 disabled:opacity-40" aria-label={`Increase quantity of ${line.name}`}>
                      +
                    </button>
                  </div>
                  {line.qty >= line.maxQty && <span className="text-xs text-ink-500">Maximum available</span>}
                  <button type="button" onClick={() => remove(line.id)} className="inline-flex items-center gap-1.5 text-[13px] text-ink-500 hover:text-rose-600" aria-label={`Remove ${line.name} from cart`}>
          ...
          <div className="rounded-xl border border-ink-200 bg-ink-50 p-6">
            ...
            <dl className="mt-5 grid gap-3 text-sm">
              <div className="flex justify-between gap-4">
                ...
                <dd className="font-medium tabular-nums text-ink-950">{formatExact(subtotal)}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-ink-600">Shipping, insurance &amp; tax</dt>
                <dd className="text-ink-600">Calculated at checkout</dd>
              </div>
            </dl>
            <p className="mt-4 text-[13px] leading-relaxed text-ink-600">Enter your address at checkout to see insured shipping options, any applicable tax, and to apply a coupon.</p>
